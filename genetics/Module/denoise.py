...
def choose_point(m, n, V, H, big, longest):
    ...
    1. V, H: list or np.array
        ...
        You should get these two from V, H = geometric_sequence(protein, dom)
    ...
    for i in range(num_rect):
        ...
        V_protein = big.loc[(big['proteinRank'] <= V[i+m-1]) & (big['proteinRank'] > V[i+m])]
        for j in range(longest): #0th_dom ~ (longest-1)_th_dom
            H_dom = V_protein[(V_protein[str(j) + "th_dom_rank"] <= H[i+n-1]) & (V_protein[str(j) + "th_dom_rank"] > H[i+n])]
            ind = H_dom.index #if I don't use index here, the content of points will be series not value
            for k in ind:
                points[i].append((H_dom.loc[k, 'proteinRank'], H_dom.loc[k, str(j) + "th_dom_rank"]))
...
def sep_point(m, n, points):
    ...
    In order proteins, the list [[(x_1,y_1),...]_(m,n), [(x_1,y_1),...]_(m+1,n+1), ...] will become
...
def scaling_fit(data, Rg_0, V, H, Zipf, name, FORMAT = 'pdf', Path = ''):
    ...
    3. Zipf: tuple (a, b), (str, str)
        ...
        where P(x, b) is the frequency-rank distribution of protein.
