...
def FRD_plot(name, protein, dom, x_pos = 2, y_pos = 10, FORMAT = 'png', Path = ''):
    ...
    ---Return:
        ...
        (1) FRD_protein['ab']: tuple (a_Z, b_Z), (str, str)
                parameters of P(x, b) = a_Z * x^(-b_Z)
        (2) FRD_protein['b_jac']: float
                gradient vector used for optimization
        (3) FRD_protein['neg_L']: str
    ...
    T = ([],[])
    L = 0 #total number of proteins in the txt
    ...
    FRD_protein['b_jac'] = float(res_Z.get('jac'))
    FRD_protein['neg_L'] = format(float(res_Z.get('fun')), '#.%dg' % dig_rho)
    FRD_protein['length'] = L
    FRD_protein['V_1'] = rank_M
    #-----------------------------------------
    fig, ax = plt.subplots()
