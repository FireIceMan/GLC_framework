...
3. count distribution of N components
4. save data
"""
...
def count_frequency(unit_list):
    ...
    ---Input 
        unit_list: list
            a list containing blocks or components
        
    ---Return: 
        D: set
            a dictionary mapping blocks to frequency.
    """
    D = {}
...
def decide_seq_order(unit_list):
    ...
    2. another_list: list
        a list containg non-repetitive blocks obeys the order of first appearence in the unit_list
    """
    D = {}
    another_list = []
    num = 0
...
def transfrom_blocklist_into_compolist(block_list):
    """Divide each block in the block_list into components, order preserved
    
    ---Input
        a list contains blocks
    
    ---Return
        a list contains components
    """
    compo_list = []
    for block in block_list:
        compo_list.extend(block.split('-'))
...
def info(file_name, encode = "UTF-8"):
    ...
    1. a dataframe contains blocks and their all components (big)
    2. the frequency information of components (compo) and blocks (block)
    3. the longest length of single block (longest)
    ...
    1. data_frame: pandas.DataFrame
      a big data frame contains the information of blocks and its components
      
    2. pd_compo: pandas.DataFrame
      a data frame contains the frequency information of components
      
    3. another_block: pandas.DataFrame
      a data frame contains the frequency information of blocks
      
    4. longest_L: int
      the biggest length of single block.
    
    '''
    L, longest_L = read_file(file_name, encode)
    block_freq = count_frequency(L)
    print("Successfully count block freqency!" + "(%s)" % file_name)
    
    block_seq, block_list = decide_seq_order(L)
    c_list = transfrom_blocklist_into_compolist(L)
    compo_seq, compo_list = decide_seq_order(c_list)
    compo_freq = count_frequency(c_list)
    print("Successfully count compo freqency!")
    
    pd_block= produce_data_frame(block_list, block_freq, block_seq,"block")
    another_block = pd_block.copy()
    pd_compo= produce_data_frame(compo_list, compo_freq, compo_seq,"compo")
    data_frame = produce_blockRank_compoRank_frame(pd_block, pd_compo, longest_L)
    print("Successfully build data frames!")
    
    return data_frame, pd_compo, another_block, longest_L

def N_gram_info(file_name, N, encode = "UTF-8"):
    '''This is only used to analysis N-gram blocks.
    ...
    2. pd_compo: pandas.DataFrame
      a data frame contains the information of components
    
    3. another_block: pandas.DataFrame
      a data frame contain the information of blocks
    ...
    block_seq, block_list = decide_seq_order(L)
    c_list = transfrom_blocklist_into_compolist(block_list)
    compo_seq, compo_list = decide_seq_order(c_list)
    compo_freq = count_frequency(c_list)
    print("Successfully count compo freqency!")
...
def write_to_excel(big, block, compo, name):
    """Save pandas dataFrame big, block, and compo as an excel file with the given filename
    
    ---Input
    big, block, compo: pandas.DataFrame
        Return of info()
    ...
    ---Output
        an excel file contains big, block, and compo
    
    """
    writer = pd.ExcelWriter(name + '.xlsx')
    big.to_excel(writer, 'RRD')
    block.to_excel(writer, 'block')
    compo.to_excel(writer, 'component')
    writer.save()

def geometric_sequence(block, compo):
    '''give geometric sequence {Hn} and {Vm}
    
    ---Parameters
        block, compo: pandas.DataFrame
            the output of info ()   
    ...
    V = [0 for i in range(len(set(block['blockFreq'])))]
    H = [0 for i in range(len(set(compo['compoFreq'])))]
    
    Vf = sorted(set(block['blockFreq']))
    Hf = sorted(set(compo['compoFreq']))
    
    SVT = 0
    SHT = 0
    
    for i in range(len(set(block['blockFreq']))):
        #ref: Count how many values in a list that satisfy certain condition
        SV = sum(1 for cf in block['blockFreq'] if cf == Vf[i])
        SVT = SVT + SV
        V[i] = len(block['blockFreq']) - SVT
    #we need to add total kinds of blocks as V_1    
    V[:0] = (max(block['blockRank']),)    
        
    for i in range(len(set(compo['compoFreq']))):
        SH = sum(1 for wf in compo['compoFreq'] if wf == Hf[i])
        SHT = SHT + SH
        H[i] = len(compo['compoFreq']) - SHT
    #we need to add total kinds of component as H_1
    H[:0] = (max(compo['compoRank']),)
    
    return V, H
    
def draw_RRD_plot(big, block, compo, longest, name, V, H, need_line = 'Y', number_of_lines = 4, Color = '#ff0000', FORMAT = 'png', Path = ''):
    '''draw the RRD plot and auxiliary lines
    
    ---Input
    1. big, block, compo, longest: pandas.DataFrame
        the return of info()
    ...
    if need_line == 'Y':
        ...
        x_range = np.linspace(0, len(block), Slice_number)
        y_range = np.linspace(0, len(compo), Slice_number)
    ...
    coordinate = []
    str_position = [i + 1 for i in range(len(big["0th_compo_rank"]))] #position starting form 1 not 0
    for i in range(longest):
        plt.plot(str_position, big[str(i) + "th_compo_rank"], 'o', markersize=3, color = Color, alpha = 0.7)
        for j in range(len(str_position)):
            if  math.isnan(big.loc[j, str(i) + "th_compo_rank"]) == False:
                coordinate.append((str_position[j], int(big.loc[j, str(i) + "th_compo_rank"])))
    ...
    plt.xlabel('block', size = 25)
    plt.ylabel('component', size = 25)
    plt.xlim([0, max(block['blockRank'])*11/10])
    plt.ylim([0, max(compo['compoRank'])*17/15])
...
def N_compo_dist(name, big, longest, density = True, FORMAT = 'png', Path = ''):
    '''N-compo means there are N components in one block, it can be 1, 2, 3..., etc. 
    ...
    ---Output
        show or save a N-compo distribution plot
        
    ---Return
    1. if density == True: N_dist_density
        1D array, N-component distribution in density form 
        sum(N_dist_density) = 1
            
    2. elif density == False: N_dist
        1D array, N-component distribution in counts form 
        sum(N_dist) = total number of component
    ...
    for i in range(len(freq)):
        N_dist[N_compo[i] - 1] += freq[i]
    
    if density == True:
        #I don't write N_dist[N_compo[i] - 1] += freq[i]/tot because such calculation is not precise
        tot = sum(freq)
        
        N_dist_density = [i/tot for i in N_dist]
            
        plt.bar(bar_x, N_dist_density, width = 1)
    ...
    plt.xlabel('$N-$component', size = 20)
    ...
    try:
        if Path == '':
            fig.savefig('N-compo of ' + name + '.' + FORMAT, dpi = 300, format = FORMAT)
            plt.show()
        else:
...
def which_plot(name, V, H, x = 'H', max_range = 50, shift = 'N', FORMAT = 'png', Path = ''):
    ...
    2. V, H: list or np.array
       ...
       where geometric_sequence(block, compo) is the function of count.py
...
def FRD_plot(name, block, compo, x_pos = 2, y_pos = 10, FORMAT = 'png', Path = ''):
    ...
    ---Return:
        FRD_block: dict, where
        (1) FRD_block['ab']: tuple (a_Z, b_Z), (str, str)
                parameters of P(x, b) = a_Z * x^(-b_Z)
        (2) FRD_block['b_jac']: float
                gradient vector used for optimization
        (3) FRD_block['neg_L']: str
                negative max liklihood.
                details see Curve_Fitting_MLE.py > L_Zipf_Mandelbrot()
        (4) FRD_block['length']: int
                total number of blocks in the txt
        (5) FRD_block['V_1'] = int
                total kinds of blocks in the txt
    ...
    #-----------------------------------------
    #T = ([blockRank], [blockFreq])
    T = ([],[])
    L = 0 #total number of blocks in the txt
    for i in block['blockRank']:
        T[0].append(i)
    for i in wf:
        L += i
        T[1].append(i)
    ...
    FRD_block = {}
    FRD_block['ab'] = (a_Z, b_Z)
    FRD_block['b_jac'] = float(res_Z.get('jac'))
    FRD_block['neg_L'] = format(float(res_Z.get('fun')), '#.%dg' % dig_rho)
    FRD_block['length'] = L
    FRD_block['V_1'] = rank_M
    #-----------------------------------------
    fig, ax = plt.subplots()
    ...
    plt.plot(wf, 'ro', label = 'block', markersize=4)
    plt.plot(cf, 'x', label = 'component', markersize=6)
...
def read_file_generate_fake_constraint(constraint = 5, compo_num = 2, out_file =  'fake1.txt', sample_block_num = 8000,
                            num_block_in_fake_scrip = 15000, 
                            alpha = 1.00001, noun = False):
    """Read "roc2.txt" file, and then generate a fake script satisfying Zipfs' law. All the blocks in 
    the output script share the same lenth compo_num
    """
    CONSTRAINT = constraint
    SAMPLE_block_NUM = sample_block_num
    ALPHA = alpha
    NUM_block_IN_NOV = num_block_in_fake_scrip
    OUTPUT_FILE_NAME = out_file
    NOUN = noun
    compo_NUM = compo_num
    
    zipf_gen =  ZipfGenerator(SAMPLE_block_NUM,ALPHA)
    f =  open("roc2.txt","r")
    ...
    list_2 = []
    tmp = ''
    for i in range(SAMPLE_block_NUM):
        for j in range(compo_num):
            c = random.choice(tmp_list)
            compo_count_dic[c] += 1
            if compo_count_dic[c] >= CONSTRAINT:
                tmp_list.remove(c)
            tmp = tmp + c
        list_2.append(tmp)
        tmp = ''
    ...
    print("blocks in corpus: " ,len(world_list))
    ...
    for i in range(NUM_block_IN_NOV):
        num = zipf_gen.next()
...
def read_file_generate_fake(compo_num = 2, out_file =  'fake1.txt', sample_block_num = 8000,
                            num_block_in_fake_scrip = 15000, 
                            alpha = 1.00001, noun = False):
    """Read "roc2.txt" file, and then generate a fake script satisfying Zipfs' law. All the blocks in 
    the output script share the same lenth compo_num
    ...
    for e in tmp_list:
        tmp = tmp + e
        if len(tmp) == compo_num:
            list_2.append(tmp)
            tmp = ''
    
    world_list = list_2

    print("blocks in a corpus: " ,len(world_list))
    ...
    random.shuffle(world_list)
    small_world_list = world_list[-SAMPLE_block_NUM:]
    target_string_list = []
