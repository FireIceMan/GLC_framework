            ...
            for word in l:
                ...
                if len(new_word) != 0: 
                    if len(new_word) > longest:
                        longest = len(new_word) #max number of syllagrams in a word
                    word_list.append(new_word)
                    new_word = ''
...
def count_allo(pdframe1, pdframe2, feature1 = "word", feature2 = "syl", feature3 = "sylFreq"):
    ...
    word_array = pdframe1[feature1] #ex: word_array=['哈囉','你好','哈哈']
    syl_array = pdframe2[feature2] #ex: syl_array=['你', '囉', '好', '哈']
    ...
    for c in syl_array: 
        allocation[c] = 0
        for w in word_array: 
            if c in w: #ex: '你' in '你好', but '你' not in '哈囉'
                allocation[c] += 1
    ...
    for w in word_array:
        ...
        for c in set(w):
            ...
            #ex: chain('哈哈') = allocation('哈') but not 2*allocation('哈')
