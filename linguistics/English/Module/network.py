...
def plot_degree_compo(name, compo_degree_sequence, FORMAT = 'pdf', Path = ''):
    ...
    b_dig = min(x_dig, y_dig) #significant figures of exponent b, it comes from log(y)/log(x)
    ...
    A = format(a_ZM, '#.%dg' % b_dig)  # give b_dig significant digits
    ...
    #record parameters
    degree_component = {}
    degree_component['abc'] = (A, B, C)
    degree_component['bc_jac'] = tuple(res_ZM.get('jac'))
    degree_component['neg_L'] = format(res_ZM.get('fun'), '#.%dg' % b_dig)
    ...
    plt.ylabel('Frequency $P(d_c)$', size = 25)
